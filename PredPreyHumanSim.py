...
Prof. Ionut Cardei
COP4045
Problem 3: Predator-Prey-Human Simulation
"""
...
class Human(Animal):
    ...
    def clock_tick(self):
        ''' Human updates breeding, starving, and hunting times
        '''
        self.breed_clock -= 1
        self.starve_clock -= 1
        self.hunt_clock -= 1
        # print('Tick, Human at {},{} starve:{}, breed:{}'.format( \
        #       self.x,self.y,self.starve_clock,self.breed_clock))
        if self.starve_clock <= 0:
    ...
    def eat(self):
        ...
        Will not kill Prey.
        '''
        if not self.moved and self.hunt_clock == 0: #checks the hunt timer along with the move check
            location = self.check_grid(Predator)
            if location:
                # print('Eating: human at {},{}, pred at {},{}'.format( \
                #       self.x,self.y,location[0],location[1]))
                self.island.remove(self.island.animal(location[0],location[1]))
                self.island.remove(self)
                self.x=location[0]
...
###########################################
def main(human_breed_time = 25, human_starve_time = 30, human_hunt_time = 10 ,initial_humans = 5, predator_breed_time=6, predator_starve_time=3, initial_predators=10, \
         prey_breed_time=3, initial_prey=50, size=10, ticks=1000):
    ...
    for i in range(ticks):
        ...
        if human_count == 0:
            ...
            break
        if prey_count == predator_count == human_count:
            print ('Numbers between Prey, Predator and Human match. Quitting.')
            ticks = i
            break
